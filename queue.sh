...
# 4. produce to queue
for i in {1..20}; do if [[ ${i}%2 -eq 0 ]]; then state="on"; else state="off"; fi; MESSAGE="{\"id\":\"`uuidgen`\",\"at\":\"`date -u +"%Y-%m-%dT%H:%M:%S.%3N"Z`\",\"state\":\"${state}\"}" ; curl -i -H 'Content-Type:application/json' -XPOST 'http://localhost:4000/queue/events' -d ${MESSAGE}; sleep 0.5s; if [[ ${i} == 10 ]]; then pkill -f "server-0.conf"; fi done
